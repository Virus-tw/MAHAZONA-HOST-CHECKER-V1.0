...
def display_header(title="MAHAZONA HOST CHECKER (V1.0 - Generic)"):
    ...
    print(f"               {Colors.RED}M{Colors.CYAN}AHAZONA HOST CHECKER (V1.0){Colors.END}")
    print(f"{Colors.YELLOW}============================================================{Colors.END}") 
...
def resolve_txt_records(hostname):
    """
    Resolves TXT records for a given hostname.
    Useful for finding verification records or custom text data.
    """
    try:
        resolver = dns.resolver.Resolver()
        resolver.nameservers = ['8.8.8.8', '1.1.1.1']
        answers = resolver.resolve(hostname, 'TXT', search=True)
        txt_records = [b.decode() for rdata in answers for b in rdata.strings]
        return txt_records, None
    except dns.resolver.NXDOMAIN:
        return None, f"No TXT records found for {hostname} (NXDOMAIN)."
    except Exception as e:
        return None, f"Error resolving TXT records: {e}."
...
def get_custom_headers():
    ...
    if load_file in ('yes', 'y'):
        ...
        try:
            ...
            print(f"{Colors.RED}Error: File '{file_path}' not found. Skipping file load.{Colors.END}")
        except Exception as e:
...
# --- NEW Reverse DNS Lookup Mode ---
def mode_reverse_dns_lookup():
    """Mode to find hostnames associated with a single IP address."""
    display_header("Reverse DNS Lookup Mode")
    print(f"{Colors.CYAN}Enter the IP address (e.g., 203.0.113.5):{Colors.END}")
    ip_addr = input(f"{Colors.GREEN}>> {Colors.END}").strip()
    ...
    print(f"\n{Colors.YELLOW}Starting Reverse DNS lookup for {ip_addr}...{Colors.END}")

    try:
        # Create the reverse lookup name (PTR record format)
        reverse_name = ipaddress.ip_address(ip_addr).reverse_pointer
        ...
        if hostnames:
            print(f"\n{Colors.GREEN}Successfully found {len(hostnames)} hostname(s):{Colors.END}")
            for hostname in hostnames:
                print(f"  {Colors.CYAN}-->{Colors.END} {hostname}")
        else:
            print(f"{Colors.YELLOW}No PTR records found for this IP address.{Colors.END}")

    except dns.resolver.NXDOMAIN:
        print(f"{Colors.YELLOW}No Reverse DNS entry (PTR record) found for {ip_addr}.{Colors.END}")
    except Exception as e:
        print(f"{Colors.RED}An error occurred during Reverse DNS lookup: {e}{Colors.END}")
...
def main_menu_selection():
    ...
    while True:
        ...
        modes = {
            ...
            3: {"name": "Reverse DNS Lookup (Find Hosts on IP)"},
        }
